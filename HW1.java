...
/*
 * *** Devon Griffith / COMP 272/440C-001 ***
 *
 ...
 public class HW1 {
     ...
     static class LinkedList {
         ...
         public void removeElementsLT ( int ltValue ) {
             ...
             while(current != null) {
                if (current.data < ltValue) {           //if current node's data < value passed
                    removeElement(current.data);        //remove it
                }
                current = current.next;                 //set current node to point to next node
             }
         ...
         public void removeElement ( int value ) {
             ...
             // YOUR CODE GOES HERE
             while (head != null && head.data == value) {       //while head != null && head's data == value
                head = head.next;                               //set current head node to point to next node 
             }
             
             Node current = head;

             while (current != null && current.next != null) {  //while not null & next not null
                if (current.next.data == value) {               //if next node's data = value
     ...
     static class Stacks {
         ...
         public static boolean isPalindrome(String input) {
             ...
             for (int i = 0; i < halfLength; i++) {
                stack.push(input.charAt(i));                //push half of word into stack
             }
                    //startInd set depending on word size is even or odd
             int startInd = (input.length() % 2 == 0) ? halfLength : halfLength + 1;
                    //loop through, if empty or stack != char; then return false
             ...
             return stack.isEmpty();            //stack is empty
         }
